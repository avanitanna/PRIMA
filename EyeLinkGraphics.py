...
class EyeLinkGraphics(custom_display):
    ...
    def __init__(self, tracker, win):
        ...
        # drawing properties
        self.xc = self.win.size[0] / 2
        self.yc = self.win.size[1] / 2
        ...
        self.target = visual.PatchStim(self.win, tex=None, mask='circle', units='pix', pos=(0, 0), size=(50, 50),
                                       color=[1, 1, 1])  # calibrating dot
    ...
    def draw_menu_screen(self):
        ...
        visual.TextStim(self.win, "Eyelink calibration menu",
                        pos=(self.xc - self.win.size[0] / 2, self.yc - 6 * self.ld - self.win.size[1] / 2),
                        height=int(2 * self.fontsize)).draw()
        visual.TextStim(self.win, "Press C to calibrate",
                        pos=(self.xc - self.win.size[0] / 2, self.yc - 3 * self.ld - self.win.size[1] / 2),
                        height=self.fontsize).draw()
        visual.TextStim(self.win, "Press V to calibrate",
                        pos=(self.xc - self.win.size[0] / 2, self.yc - 2 * self.ld - self.win.size[1] / 2),
                        height=self.fontsize).draw()
        visual.TextStim(self.win, "Press A to auto-threshold",
                        pos=(self.xc - self.win.size[0] / 2, self.yc - 1 * self.ld - self.win.size[1] / 2),
                        height=self.fontsize).draw()
        visual.TextStim(self.win, "Press Enter to show camera image",
                        pos=(self.xc - self.win.size[0] / 2, self.yc - 0 * self.ld - self.win.size[1] / 2),
                        height=self.fontsize).draw()
        visual.TextStim(self.win, "Press Escape to continue to the experiment",
                        pos=(self.xc - self.win.size[0] / 2, self.yc + 1 * self.ld - self.win.size[1] / 2),
                        height=self.fontsize).draw()
    ...
    def draw_cal_target(self, x, y):
        ...
        """
        self.show_menu = False
        self.target.setPos((x - 0.5 * self.win.size[0], 0.5 * self.win.size[1] - y))
        self.target.draw()
        self.win.flip()
    ...
    def draw_image_line(self, width, line, totlines, buff):
        ...
        if line == totlines:
            ...
            visual.ImageStim(self.win, self.tmp_file, pos=(0, 0))
